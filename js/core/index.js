...
setTimeout(window.addEventListener, 50, 'load', () => {
...
const fallBackName = "junk" //fallback for image search by name
...
const input = {
  ...
  focus: null,
  setTextFocus() {
    const backgroundColor = getComputedStyle(document.querySelector(':root')).getPropertyValue('--bg-color')
...
function onHistoryInputChange(e) {
  if (simulation.testing && simulation.isChatMenuOpen && cmdConsole.isHistoryInputFocused) {
    let currentValue = parseInt(e.target.value), minValue = 0, maxValue = cmdConsole.history.length;
    if (isNaN(currentValue)) {
      e.value = minValue; // Reset to min if input is invalid
      return;
    }

    if (currentValue > maxValue) {
      e.target.value = minValue; // Wrap around to min if exceeding max
    } else if (currentValue < minValue) {
      e.target.value = maxValue; // Wrap around to max if going below min
    }
  }
}
  ...
  if (simulation.isChatMenuOpen) { //if player is typing in chat, ignore keypresses
    ...
    switch (event.code) {
      ...
      case input.key.testing:
        if (m.alive && localSettings.loreCount > 0 && !build.isExperimentSelection) {
          ...
          if (simulation.testing) {
            ...
            if (simulation.isConstructionMode) {
              ...
              simulation.inGameConsole(
                ...
                <tr>
                    <td class='key-input-pause'>L</td>
                    <td class='key-used'>show / hide images</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td class='key-input-pause'>0-9</td>
                    <td class='key-used'>spawn things</td>
                </tr>
                <tr>
                    <td class='key-input-pause'>Enter</td>
                    <td class='key-used'>Open chat</td>
                </tr>
                <tr>
                    <td class='key-input-pause'>⇧X or Ctrl+W</td>
    ...
    if (simulation.testing) {
      ...
      if (event.key === "=") {
        powerUps.directSpawn(simulation.mouseInGame.x, simulation.mouseInGame.y, "levelList");
      }
      if (event.key === "*") {
