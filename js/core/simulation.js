...
//*********************************************************************
setTimeout(() => {
fileLoads.isSimulationJS = true; //for file handling in fileTester.js
}, 10);
...
testingLoop() {
  ...
  simulation.wipe();
  simulation.textLog();
  if (!simulation.isChatMenuOpen) { //Freeze player in place while they're typing in chat
    if (m.onGround) {
      m.groundControl()
    } else {
      m.airControl()
  ...
  if (input.fire && m.fireCDcycle < m.cycle) {
    ...
    for (let i = 0, len = mob.length; i < len; i++) {
      if (!mob[i].shield) {
        let vect = Vector.sub(mob[i].position, simulation.mouseInGame)
        if (Vector.magnitude(vect) < mob[i].radius) {
          //console.log(mob[i])
          if (mob[i].name) {
            simulation.inGameConsole(`m<span class='color-symbol'>.</span>lookingAt<span class='color-symbol'>(</span> 
            mob<span class='color-symbol'>[</span>i<span class='color-symbol'>] ).</span>name <span class='color-symbol'>=</span> 
              ${mob[i].name}`)
          } else {
            simulation.inGameConsole(`<strong style='color:red'>UNCAUGHT ERROR:</strong> m<span class='color-symbol'>.</span>lookingAt<span class='color-symbol'>(</span> 
            mob<span class='color-symbol'>[</span>i<span class='color-symbol'>] )</span> is unknown`)
          }
        }
      }
    }
  ...
  simulation.drawCircle();
  simulation.runEphemera();
  if (!simulation.isChatMenuOpen) simulation.constructCycle();
  ctx.restore();
  simulation.testingOutput();
  if (!simulation.isChatMenuOpen) simulation.drawCursor();
},
...
closeChatWindow() {
  try {
    cmdConsole.cmdIdx = 0
    //simulation.lastLogTime = m.cycle + 240 //close chat window after 4 seconds
    //simulation.lastTextLog = null
  ...
  simulation.ephemera.push({
    name: "uniqueName", count: 0, do() {
      if (!(m.cycle % 60)) { //once a second
        ...
        if (isNaN(player.position.x)) { //go to start
            Matter.Body.setVelocity(player, { x: 0, y: 0 });
            Matter.Body.setPosition(player, { x: level.enter.x + 50, y: level.enter.y - 20 });
            // move bots
            for (let i = 0; i < bullet.length; i++) {
