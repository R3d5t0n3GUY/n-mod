setTimeout(() => {
  fileLoads.isBulletJS = true; //for file handling in fileTester.js
}, 10);
...
const b = {
  ...
  explosion(where, radius, color = "rgba(255,25,0,0.6)", reducedKnock = 1) { // typically explode is used for some bullets with .onEnd
    ...
    try {
      if (tech.isSounds) {
        audioPlayer.requestSound('Explosion', where);
      }
    } catch (err) {
      simulation.inGameConsole(`Unable to execute sound player:<br>${err}`);
    }
  ...
  missile(where, angle, speed, size = 1) {
    ...
    bullet[me] = Bodies.rectangle(where.x, where.y, 30 * size, 4 * size, {
      ...
      explodeRad: ((tech.isMissileBig ? 230 : 180) + 60 * Math.random()) *
        (tech.isMissileBotUpgrade ? 2 ** tech.isMissileBotUpgrade : 1),
  ...
  worm(where, isFreeze = tech.isSporeFreeze) { //used with the tech upgrade in mob.death()
    ...
    if (bIndex < 500) { //can't make over 500 spores
      bullet[bIndex] = Bodies.polygon(where.x, where.y, 3, 3, {
        ...
        beforeDmg(who) {
          if (who.isInvulnerable || who.isZombie) {
            Matter.Body.setVelocity(this, Vector.mult(this.velocity, 0.1));
          } else if (tech.isZombieWorm) {
            if ((who.health - this.dmg) < 0.01 || isNaN(who.health) && who.alive) {
              for (let i = 0, len = tech.tech.length; i < len; i++) {
                if (tech.tech[i].name === "zombie worms") {
                  if (tech.tech[i].count >= 5) {
                    who.isSoonZombie = true;
                  } else if (Math.random() < tech.tech[i].count * 0.2) {
                    who.isSoonZombie = true;
                  }
                }
          ...
          } else if (tech.isSpawnBulletsOnDeath && who.alive && who.isDropPowerUp) {
            setTimeout(() => {
              if (!who.alive) {
                for (let i = 0, len = tech.tech.length; i < len; i++) {
                  if (tech.tech[i].name === "necrophage") {
                    for (let j = 0; j < 3 * tech.tech[i].count; j++) { //spawn 3 more for each stack level
  ...
  foam(position, velocity, radius) {
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 20, radius, {
      ...
      damage: tech.foamDamage * (tech.isFastFoam ? 2.8 : 1) * (tech.isBulletTeleport ? 1.53 : 1) * (tech.isFoamExplode ? 1.33 : 1), //damage done over time
  ...
  stabSword(where, angle = m.angle) {
    const me = bullet.length;
  },
  throwScythe(where, angle = m.angle) {
    const me = bullet.length;
  },
  orbitScythe(where, angle = m.angle) {
    const me = bullet.length;
  },
  ...
  spearDmgBeam(where, angle = m.angle) {
    const me = bullet.length;
  },
  spearLightning(start, end) {
    const me = bullet.length;
  },
  ferroFluidStream(where, angle = m.angle) {
    const me = bullet.length;
  },
  ferroFluidBall(where, angle = m.angle) {
    const me = bullet.length;
  },
  sniperShell(where, angle = m.angle) {
    const me = bullet.length;
  },
  ...
  missileBot(position = { x: player.position.x + 50 * (Math.random() - 0.5), y: player.position.y + 50 * (Math.random() - 0.5) }, isKeep = true) {
    ...
    bullet[me] = Bodies.rectangle(position.x, position.y, 28, 11, {
      isUpgraded: tech.isMissileBotUpgrade,
      botType: "missile",
      angle: m.angle,
      friction: 0,
      frictionStatic: 0,
      ...
      cd: 0,
      delay: Math.floor(60) * (0.5 ** tech.isMissileBotUpgrade),
  ...
  plasmaBot(position = { x: player.position.x + 50 * (Math.random() - 0.5), y: player.position.y + 50 * (Math.random() - 0.5) }, isKeep = true) {
    ...
    bullet[me] = Bodies.polygon(position.x, position.y, 5, RADIUS, {
      isUpgraded: tech.isPlasmaBotUpgrade,
      botType: "plasma",
      angle: dir,
      friction: 0,
      frictionStatic: 0,
      ...
      do() {
        ...
        if (this.lockedOn && this.lockedOn.alive && m.fieldCDcycle < m.cycle) {
          ...
          if (DIST < tech.isPlasmaRange * 450 && m.energy > this.drainThreshold) {
            ...
            if (this.isUpgraded) {
              for (let i = 0, len = tech.tech.length; i < len; i++) {
                if (tech.tech[i].name === "plasma-bot upgrade") {
                  drain *= 0.5 ** tech.tech[i].count;
                  break;
                }
              }
            }
            ...
            if (best.dist2 != Infinity) { //if hitting something
              ...
              if (best.who.alive) {
                let dmg = 1.4;
                if (this.isUpgraded) {
                  dmg *= 2.0 ** tech.isPlasmaBotUpgrade.count;
                }//********** SCALE DAMAGE HERE *********************
                best.who.damage(dmg);
                best.who.locatePlayer();
                //push mobs away
  ...
  guns: [
        ...
        const chooseBulletType = function () {
          if (tech.isLaserShot) {
            simulation.ephemera.push({
              ...
              do() {
                this.count--
                if (this.count < 0 || this.cleared !== level.levelsCleared) simulation.removeEphemera(this.name)
    ...
    },
    {
      ...
      fire() {
        ...
        //     }
        // } else {
        m.fireCDcycle = m.cycle + tech.missileFireCD * b.fireCDscale * (0.5 ** tech.isMissileBotUpgrade) / countReduction; // cool down
      ...
      fire() {
        if (tech.isSounds) {
          audioPlayer.requestSound('Fire in the hole', m.pos);
        }
          ...
          for (let i = 0; i < this.bladeTrails.length; i++) {
            ...
            for (let j = 0; j < trail.length; j++) {
              ...
              if (tech.isEnergyHealth) {
                ...
                const color = `#${r}${r}${g}${g}${b}${b}${Math.round(alpha * 255).toString(16).padStart(2, '0')}`;
                ctx.fillStyle = color;
              } else if (tech.isAmmoScythe && tech.isStunScythe) {
                ctx.fillStyle = `#004010${Math.round(alpha * 255).toString(16).padStart(2, '0')}`
      ...
      fire() {
        ...
        const spray = (num) => {
          ...
          for (let i = 0; i < num; i++) {
            ...
            bullet[me].endCycle = simulation.cycle + 500
            bullet[me].minDmgSpeed = 1
            // bullet[me].restitution = 0.4
            ...
            bullet[me].do = function () {
              try {
                ...
                //Matter.Body.setAngle(this.dir)
                if (height < 10) {
                  //simulation.inGameConsole("END!")
                  this.endCycle = 0
                }
              } catch (err) {
                this.endCycle = 0
