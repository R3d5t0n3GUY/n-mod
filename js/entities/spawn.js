setTimeout(() => {
  fileLoads.isSpawnJS = true; //for file handling in fileTester.js
}, 10);
...
const spawn = {
  ...
  bossTier: [
    ...
    ["sneakBoss", "blockBoss", "laserTargetingBoss", "blinkBoss", "pulsarBoss", "spawnerBossCulture", "growBossCulture", "spiderBoss2", "tendrilBoss", "hydraBoss2", "mayflyBoss"],
    ["powerUpBossBaby", "laserLayerBoss", "historyBoss", "beetleBoss", "snakeSpitBoss", "mantisBoss", "laserBombingBoss", "cellBossCulture", "bomberBoss", "timeSkipBoss", "spiderBoss3", "tendrilBoss3", "hydraBoss3"],
  ...
  forefitList: [ //joke lines powerUp-type bosses say when spawned on level.final
    ...
    `Give this bounty on M's head to someone else, eh? Your fat a** takes up too much room here.`,
    `Where'd you get your shield, finalBoss? From the dump? No wonder you smell of sh*t.`,
    `Man, they make anyone the final boss these days. Don't wanna be the guy to be sat on, fat one.`,
  ...
  finalBoss(x, y, radius = 300) {
    ...
    simulation.inGameConsole(level.levels[level.onLevel]);
    ...
    me.lasers = function (where, angle, dmg = 0.1 * me.damageScale()) {
      ...
      if (best.who && (best.who === playerBody || best.who === playerHead) && m.immuneCycle < m.cycle) {
        if (m.immuneCycle < m.cycle + 60 + m.collisionImmuneCycles) m.immuneCycle = m.cycle + 60 * ((m.fieldMode === 0 || m.fieldMode === 14) ? m.immuneBoostCouple : 1) + m.collisionImmuneCycles; //player is immune to damage extra time
        m.takeDamage(dmg);
        simulation.drawList.push({ //add dmg to draw queue
          x: best.x,
          y: best.y,
    ...
    me.onDeath = function () {
      ...
      if (!this.hasRunDeathScript) {
        ...
        if (localSettings.loreCount > 5 || lore.techCount > (lore.techGoal - 1) && !simulation.isCheating) {
          function lorePush() {
            simulation.inGameConsole(`level.levels.push("<span class='lore-text'>undefined</span>")`, 720);
            unlockExit()
        ...
        if (tech.isSounds) {
          audioPlayer.requestSound("finalBossDeath")
        }
        ...
        const hit = (who) => {
          if (!who.isZombie && who.damageReduction) {
            ...
            if ((who.health < 0.01 || isNaN(who.health)) && who.alive) {
              if (tech.isZombieInfection) {
                for (let i = 0, len = tech.tech.length; i < len; i++) {
                  if (tech.tech[i].name === "The Apocalypse") {
                    if (tech.tech[i].count >= 5) {
                      who.isSoonZombie = true;
                    } else {
                      if (Math.random() < tech.tech[i].count * 0.2) {
                        who.isSoonZombie = true;
                      }
                    }
                  }
  ...
  ringwormBoss(x, y) {
    ...
    me.laserSpear = function (where, angle) {
      ...
      if (best.who && (best.who === playerBody || best.who === playerHead)) {
        ...
        if (m.immuneCycle < m.cycle && !m.isCloak) {
  ...
  ringwormBoss4(x, y) {
    ...
    me.do = function () {
      ...
      if (this.distanceToPlayer() < 500) {
        if (!this.isSlashing && m.immuneCycle < m.cycle && !m.isCloak && Matter.Query.ray(map, this.position, m.pos).length === 0) this.sword = this.swordWaiting
      }
      this.attraction();
      this.sword() //does various things depending on what stage of the sword swing
      if (this.isInvulnerable) {
  ...
  powerUpBossBaby(x, y, vertices = 9, radius = 60, refunds = 0) {
    mobs.spawn(x, y, vertices, radius, "rgba(225,240,245,0.4)"); //"rgba(120,140,150,0.4)"
    let me = mob[mob.length - 1];
    me.name = "powerUpBossBaby"
    me.tier = 3
    me.techRefunds = refunds
    me.isBoss = true;
    me.frictionAir = 0.006
    ...
    if (level.levels[level.onLevel] == `final`){
      let tauntString = spawn.forefitList[Math.floor(Math.random() * spawn.forefitList.length)];
      simulation.inGameConsole(`<strong style="color:red">powerUpBossBaby</strong>: `+tauntString);
      me.death();
    }
    if (vertices === 9) {
      //on primary spawn
      ...
      if (tech.isEnergyNoAmmo) {
        powerUps.spawn(me.position.x, me.position.y, "research");
        powerUps.spawn(me.position.x, me.position.y, "research");
      } else {
  ...
  powerUpBoss(x, y, vertices = 9, radius = 130, refunds = 0) {
    ...
    me.frictionAir = 0.01
    me.seeAtDistance2 = 1000000;
    me.accelMag = 0.0001 + 0.0002 * simulation.accelScale;
    ...
    if (level.levels[level.onLevel] == `final`){
      let tauntString = spawn.forefitList[Math.floor(Math.random() * spawn.forefitList.length)];
      simulation.inGameConsole(`<strong style="color:red">powerUpBoss</strong>: `+tauntString);
      me.death();
    }
    if (vertices === 9) {
      //on primary spawn
    ...
    me.startingDamageReduction = me.damageReduction
    me.damageReduction = 0
    me.invulnerabilityCountDown = 15 + simulation.difficulty / 2
  ...
  kingSnakeBoss(x, y) {
    ...
    me.do = function () {
      ...
      if (m.immuneCycle < m.cycle) {
        for (let i = 0; i < this.history.length - 1; i++) {
          if (Matter.Query.ray([player], this.history[i], this.history[i + 1], 10).length > 0) {
            m.immuneCycle = m.cycle + m.collisionImmuneCycles + 60 * ((m.fieldMode === 0 || m.fieldMode === 14) ? m.immuneBoostCouple : 1)
            const dmg = 0.15 * this.damageScale()
            m.takeDamage(dmg);
            simulation.drawList.push({ //add dmg to draw queue
              x: m.pos.x,
            ...
            simulation.ephemera.push({
              ...
              do() {
                ...
                if (Vector.magnitude(Vector.sub(player.position, this.position)) < this.radius && m.immuneCycle < m.cycle) {
                  m.immuneCycle = m.cycle + m.collisionImmuneCycles + 30 * ((m.fieldMode === 0 || m.fieldMode === 14) ? m.immuneBoostCouple : 1); //player is immune to damage
      ...
      if (this.seePlayer.recall) {
        ...
        if (simulation.cycle % this.laserInterval > this.laserInterval / 2) {
          for (let i = 0; i < 5; i++) {
            ...
            // hitting player
            if ((best.who === playerBody || best.who === playerHead) && m.immuneCycle < m.cycle) {
              m.immuneCycle = m.cycle + m.collisionImmuneCycles + 30 * ((m.fieldMode === 0 || m.fieldMode === 14) ? m.immuneBoostCouple : 1); //player is immune to damage for an extra second
  ...
  revolutionBoss(x, y, radius = 70) {
    ...
    me.laserSword = function (where, angle, length) {
      ...
      if (best.who && (best.who === playerBody || best.who === playerHead) && m.immuneCycle < m.cycle && !m.isCloak) {
