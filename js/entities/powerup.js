setTimeout(() => {
  fileLoads.isPowerUpJS = true; //for file handling in fileTester.js
}, 10);
...
const powerUps = {
  ...
  debugRerolls(type) {
    ...
    if (type === "gun") {
      ...
      simulation.inGameConsole(`${powerUps.orb.field()}<strong class='color-f'>field</strong> <strong class='color-cancel'>cancel</strong> count: ${tech.cancelFieldCount}`);
      simulation.inGameConsole(`<span class='color-var'>tech</span>.canFieldReroll <span class='color-symbol'>=</span> ${tech.canFieldReroll}`);
    } else if (type === "tech") {
      simulation.inGameConsole(`${powerUps.orb.tech()}<<strong class='color-tech'>tech</strong> <strong class='color-cancel'>cancel</strong> count: ${tech.cancelTechCount}`);
      simulation.inGameConsole(`<span class='color-var'>tech</span>.canTechReroll <span class='color-symbol'>=</span> ${tech.canTechReroll}`);
    }
  },
  endDraft(type, isCanceled = false) { //type should be a gun, tech, or field
    powerUps.choosingType = null
    if (isCanceled) {
      ...
      if (tech.isCancelTech && (
        ...
        (type === "field" && tech.cancelFieldCount < tech.isCancelCount && tech.canFieldReroll) ||
        (type === "tech" && tech.cancelTechCount < tech.isCancelCount && tech.canTechReroll)) ){
        if (type === "gun") {
          tech.cancelGunCount++;
        } else if (type === "field") {
          tech.cancelFieldCount++
        } else if (type === "tech") {
          tech.cancelTechCount++
        }
        // powerUps.research.use('tech')
        powerUps[type].effect(true);
        if (type === "gun") {
 ...
 m.setField("name")                 /*standing wave  perfect diamagnetism
 	negative mass  molecular assembler  plasma torch  time dilation
    	metamaterial cloaking  pilot wave  wormhole  grappling hook
 	tachyonic field  energy condenser  Einsteins shield  matter displacement*/
 b.giveGuns("name")                 /*nail gun  shotgun  super balls  wave
 	missiles  grenades  spores  drones  foam  harpoon  mine  laser
    	MR FerroFluid  sword  scythe  spear  sniper*/
 ...
 powerUps.spawn(m.pos.x, m.pos.y, "name") /*tech gun field heal ammo research
 	coupling boost instructions difficulty entanglement*/

 //this URL downloads newest version of n-gon (unmodded)
 ...
 <strong>Win/Linux:</strong> Ctrl + Shift + J        Ctrl + Shift + J
  ...
  warpText (levelSubList, openTag) {
    ...
    for (let i = 0; i < levelSubList.length; i++) {
        ...
        let style = localSettings.isHideImages ? powerUps.hideStyle : `style='max-height: 8rem; font-size: 1rem;padding-left:5px; background-image: ${build.getBackgroundImageURL(what, 'level')};'`
  ...
  settings: {
    ...
    effect() {
      ...
      powerUps.animatePowerUpGrab('rgba(245, 24, 149, 0.3)')
        ...
        for (let i = 0; i < keys.length; i++) {
          if (typeof(values[i]) === "object" && values[i] !== null && !Array.isArray(values[i])) { //if a value is an object, display its properties, instead of '[object Object]'
            ...
            for (let j = 0; j < subkeys.length; j++) {
              if (Array.isArray(subvalues[j])) { //if a value is an array, display its items, instead of '[object Object]'
  ...
  difficulty: {
    ...
    exit() {
      if (simulation.isTraining && (level.onLevel > level.levels.length - 1)) {
    ...
    effect() {
      ...
      if (localSettings.isAllowed) {
        if (localSettings.loreCount > 5) {
          pacifistWarn = `<br><br><strong style='color:red'>NOT RECOMMENDED</strong> for pacifist runs!`
        }
      }
      requestAnimationFrame(() => { //add a background behind the power up menu
        ctx.fillStyle = `rgba(150,150,150,0.9)`;
        ...
        <div>
            <div class="grid-container">
                ...
                <div class="right-column">
                    ...
                    <div class="row" id="constraint-7"><strong>+1</strong> random <strong class="constraint">constraint</strong><br>fewer ${powerUps.orb.tech()} spawn ${pacifistWarn}</div>
                </div>
        ...
        simulation.ephemera.push({ //call each power up that's been merged with a delay
          ...
          do() {
            if (!((m.cycle + this.cycleStart) % 10)) {
              ...
              if (this.index === 0) simulation.removeEphemera(this.name)
            }
          },
        })
      }
  ...
  heal: {
      ...
      if (!tech.isEnergyHealth && m.alive) {
        ...
        if (heal > 0) {
          ...
          if (tech.isOverHeal && overHeal > 0) { //tech quenching
            ...
            m.setMaxHealth();
            simulation.inGameConsole(`<div class="circle-grid heal"></div> &nbsp; <span class='color-var'>m</span>.maxHealth <span class='color-symbol'>=</span> ${m.maxHealth.toFixed(2)} ( <span class='color-symbol'>+=</span> ${(0.3 * overHeal).toFixed(3)})`)
    ...
    spawn(x, y, size) { //used to spawn a heal with a specific size / heal amount, not normally used
      ...
      powerUps.spawn(x, y, name, false, size)
    }
  },
  ammo: {
    name: "ammo",
  ...
  fieldText(choose, click) {
    ...
    const style = localSettings.isHideImages ? powerUps.hideStyle : `style='background-image: ${build.getBackgroundImageURL(choose, "field")};'`
    return `<div class="choose-grid-module card-background" onclick="${click}" onauxclick="${click}"${style}>
        <div class="card-text">
        <div class="grid-title"><div class="circle-grid field"></div> &nbsp; ${m.fieldUpgrades[choose].name}</div>
        ${m.fieldUpgrades[choose].descriptionFunction ? m.fieldUpgrades[choose].descriptionFunction() : m.fieldUpgrades[choose].description}</div></div>`
  ...
  techText(choose, click) {
    ...
    const style = localSettings.isHideImages || tech.tech[choose].isLore ? powerUps.hideStyle : `style='background-image: ${build.getBackgroundImageURL(choose, "tech")};'`
    return `<div class="choose-grid-module card-background" onclick="${click}" onauxclick="${click}"${style}>
                <div class="card-text">
    ...
    effect(cancelled = false) {
      if (m.alive) {
        ...
        if (options.length > 0 || !tech.isSuperDeterminism) {
          ...
          if (tech.isFlipFlopChoices && tech.isFlipFlopOn) totalChoices += 7 * (tech.flipFlopChoices ? tech.flipFlopChoices : 1)
          if (tech.isCancelTech && tech.cancelGunCount <= tech.isCancelCount &&
              tech.cancelGunCount > 0 && tech.canGunReroll && cancelled) {
            totalChoices *= 3
            tech.canGunReroll = true
            cancelled = false
          }
  ...
  field: {
    ...
    effect(cancelled = false) {
      if (m.alive) {
        ...
        let totalChoices = 2 + tech.extraChoices + (tech.isInPilot ? 1 : 3) * (m.fieldMode === 8) - level.fewerChoices
        if (tech.isFlipFlopChoices && tech.isFlipFlopOn) totalChoices += 7 * (tech.flipFlopChoices ? tech.flipFlopChoices : 1)
        if (tech.isCancelTech && tech.cancelFieldCount <= tech.isCancelCount &&
            tech.cancelFieldCount > 0 && tech.canFieldReroll && cancelled) {
          totalChoices *= 3
          tech.canFieldReroll = true
          cancelled = false
          this.effect(false);
  ...
  tech: {
    ...
    effect(cancelled = false) {
      if (m.alive) {
        ...
        if (tech.isFlipFlopChoices && tech.isFlipFlopOn) totalChoices += 7 * (tech.flipFlopChoices ? tech.flipFlopChoices : 1)
        if (tech.isCancelTech && tech.cancelTechCount <= tech.isCancelCount &&
            tech.cancelTechCount > 0 && tech.canTechReroll && cancelled) {
          totalChoices *= 3
          tech.canTechReroll = true
          cancelled = false
          this.effect(false);
  ...
  pauseEjectTech(index) {
    if ((tech.isPauseEjectTech || simulation.testing) && !(simulation.isChoosing || tech.tech[index].isInstant)) {
      if ((tech.tech[index].name === "marginal utility") && tech.isRerollGunAmmo) { //if player has cardinal utility, reroll marginal utility instead of ejecting it
        ...
        simulation.updateTechHUD();
        simulation.updateGunHUD();
      } else if (!tech.isPauseEjectResearch || powerUps.research.count >= (tech.pauseEjectResearch || 1)) {
  ...
  spawn(x, y, name, moving = true, size = 0) {
    if (name === "random") {
      ...
      while (
        ...
        (pickList[idx] === "heal" && tech.isEnergyHealth && !powerUps.healGiveMaxEnergy) || //don't pick heal if player has Mass-Energy and not 1st Ionization
        (pickList[idx] === "ammo" && tech.isEnergyNoAmmo) || //don't pick ammo if player has Non-Renewables
        (pickList[idx] === "research" && tech.isSuperDeterminism) || //don't pick research if player has Superdeterminism
        (pickList[idx] === "gun" && (b.inventory.length >= b.guns.length)) || //don't pick gun if player has all the guns
        (pickList[idx] === "field" && tech.isPauseSwitchField) //don't pick field if player has Unified Field Theory
      ) {//don't pick invalid powerups
        idx = Math.floor(Math.random() * pickList.length);
      }
      if (pickList[idx] === "ammo" && tech.isBoostReplaceAmmo) {
        name = "boost";
      } else {
        name = pickList[idx];
      }
