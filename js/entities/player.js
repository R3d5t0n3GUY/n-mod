  ...
  switchWorlds(giveTech = "") {
    ...
    if (!m.isSwitchingWorlds) {
      ...
      let loop = () => {
        if (!(m.cycle % 10)) {
          ...
          if (totalTech > 0 && m.alive) {
            ...
            if (options.length > 0) tech.giveTech(options.randomItem(weights)) //add a new tech from options pool
      ...
      if (tech.isAltRealitySpawn) {
        ...
        //random bullets
        const where = m.pos, things = [() => { //spore
            b.spore(where)
          }, () => { //worm
            b.worm(where)
          }, () => { //flea
  ...
  skin: {
      ...
      m.draw = function() {
        const ledgeJump = function(xForce) {
          if (input.up && m.buttonCD_jump + 20 < m.cycle && !m.onGround) {
            ...
            //if (tech.isCoyote) m.lastOnGroundCycle = m.cycle //tried to get acoustic levitation to trigger after wall jump. didn't work
          }
        }
        ...
        if (input.left && !m.onGround && player.velocity.y > -10 && m.cycle > m.lastOnGroundCycle + 10 &&
          m.buttonCD_jump + 20 < m.cycle && Matter.Query.ray(map, {
            x: m.pos.x - sensorWidth,
            y: m.pos.y + 50
          }, {
            x: m.pos.x - sensorWidth,
          ...
          m.yOffGoal = m.yOffWhen.stand;
          ledgeJump(0.4)
        } else if (input.right && !m.onGround && player.velocity.y > -10 && m.cycle > m.lastOnGroundCycle + 10 &&
          m.buttonCD_jump + 20 < m.cycle && Matter.Query.ray(map, {
            x: m.pos.x + sensorWidth,
            y: m.pos.y + 50
          }, {
    ...
    scaleInvariance() {
      ...
      m.draw = function () {
        ...
        ctx.globalAlpha = (m.immuneCycle < m.cycle) ? 1 : m.cycle % 3 ? 0.1 : 0.65 + 0.1 * Math.random()
        m.calcLeg(Math.PI, -3);
        if (!m.isShipMode) m.drawLeg("#4a4a4a");
        m.calcLeg(0, 0);
        if (!m.isShipMode) m.drawLeg("#333");
        ctx.rotate(m.angle);
        ctx.beginPath();
        ctx.arc(0, 0, 30, 0, 2 * Math.PI);
        ctx.fillStyle = m.bodyGradient
    ...
    dilate() {
      ...
      m.draw = function () {
        ...
        ctx.translate(m.pos.x, m.pos.y);
        m.calcLeg(Math.PI, -3);
        if (!m.isShipMode) m.drawLeg("#456");
        m.calcLeg(0, 0);
        if (!m.isShipMode) m.drawLeg("#345");
        ctx.rotate(m.angle);
        ctx.beginPath();
        ctx.arc(0, 0, 30, 0, 2 * Math.PI);
        ctx.fillStyle = m.bodyGradient
    ...
    Sleipnir() {
      m.isAltSkin = true
      m.oldDrawLeg = m.drawLeg
      m.drawLeg = function() {
        if (!m.isShipMode) for (let i = 0; i < 16; i++) {
  ...
  fieldUpgrades: [
                  ...
                  if (dist1 < suckRange) {
                    ...
                    if (Vector.magnitude(Vector.sub(m.hole.pos1, body[i].position)) < shrinkRange) {
                      ...
                      if (body[i].mass < 0.05) {
                        ...
                        body.splice(i, 1);
                        m.fieldRange *= 0.8
                        if ((m.fieldMode === 0 || m.fieldMode === 9) && m.immuneCycle < m.cycle) {
                          m.energy += 0.02 * m.coupling * level.isReducedRegen
    ...
    },
    {
      ...
      description: `use <strong class="color-f">energy</strong> to gain a <strong>burst</strong> of <strong class='color-speed'>speed</strong>
              ...
              <br>16 <strong class="color-f">energy</strong> per second <em class='color-combo'>←↑→↓↓</em>`,
      canMove: false,
      keyLog: [null, null, null, null, null],
      fieldEvent (event) {
    ...
    },
    {
      ...
      description: `use <strong class='color-f'>energy</strong> to recover <strong class='color-h'>health</strong>
            		...
            		<br><strong>6</strong> <strong class='color-f'>energy</strong> per second <em class='color-combo'>→↓←↓↓</em>`,
      keyLog: [null, null, null, null, null],
      fieldEvent (event) {
      ...
      effect: () => {
        ...
        m.hold = function () {
          if (m.isHolding) {
            ...
            m.holding();
            m.throwBlock();
          } else if (m.crouch && input.field && input.down && (m.energy > 0.25) && ((this.teleportCDcycle || 0) < m.cycle)) {
            m.energy -= 0.25
            m.resetHistory();
            Matter.Body.setPosition(player, simulation.mouseInGame);
            Matter.Body.setVelocity(player, { x: 0, y: 0 });
