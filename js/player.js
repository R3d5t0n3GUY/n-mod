    ...
    couplingDescription(couple = m.coupling){
				switch (m.fieldMode) {
                    ...
                    case 11: //tachyonic field
                     	return `<strong>${(1 + Math.abs(Math.log(m.coupling + 1)) / 5).toFixed(3)}x</strong> movement <em class="color-s">speed</em>`
          ...
          m.hold = () => {
            ...
            if (m.isHolding) {
              ...
              if (m.crouch && m.energy > DRAIN && !tech.isEnergyHealth) { //won't work with mass-energy
                ...
                if (this.loggingCDcycle < m.cycle) {
                  if (m.health < m.maxHealth) {
