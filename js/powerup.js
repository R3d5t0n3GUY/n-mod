...
const powerUps = {
  ...
  debug() {
    ...
    simulation.lastLogTime = 0;
  	simulation.inGameConsole("gun cancel count: "+tech.cancelGunCount);
    simulation.inGameConsole("canGunReroll: "+tech.canGunReroll);
    simulation.inGameConsole("field cancel count: "+tech.cancelFieldCount);
    simulation.inGameConsole("canFieldReroll: "+tech.canFieldReroll);
    simulation.inGameConsole("tech cancel count: "+tech.cancelTechCount);
    simulation.inGameConsole("canTechReroll: "+tech.canTechReroll);
    */
  },
    ...
    if (isCanceled) {
      ...
      if (tech.isCancelRerolls) {
        for (let i = 0, len = 10 + 6 * Math.random(); i < len; i++) {
          ...
          powerUps.spawn(m.pos.x + 40 * (Math.random() - 0.5), m.pos.y + 40 * (Math.random() - 0.5),
                         spawnType, false);
        }
      }
 ...
 <strong>Win/Linux:</strong> Ctrl + Shift + J        Ctrl + Shift + J
  ...
  junkTechText(choose, click) {
    const techCountText = tech.tech[choose].count > 0 ? `(${tech.tech[choose].count + 1}x)` : "";
    const style = localSettings.isHideImages ? powerUps.hideStyle : `style="background-size: contain;background-repeat: no-repeat;background-image: url('img/${tech.tech[choose].name}.webp');"`
    if (!localSettings.isHideImages) {
        ...
        if (options.length > 0) {
          ...
          if (tech.isExtraGunField) {
           ...
           if (tech.isMicroTransactions && powerUps.research.count > 0) {
              ...
              text += `<div class="choose-grid-module" onclick="tech.giveTech(${choose});powerUps.research.changeRerolls(-1);powerUps.endDraft('tech');"><div class="grid-title"><div class="circle-grid research"></div> <span style = 'font-size:90%; font-weight: 100; letter-spacing: -1.5px;'>microtransaction:</span>
              <br>${tech.tech[choose].name}</div>${tech.tech[choose].descriptionFunction ? tech.tech[choose].descriptionFunction() : tech.tech[choose].description}</div>`
          }
