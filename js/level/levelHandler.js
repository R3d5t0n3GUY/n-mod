...
const level = {
  ...
  communityLevels: ["gauntlet", "stronghold", "basement", "crossfire", "vats", "ngon", "house", "perplex",
    ...
    "tlinat", "ruins", "ace", "LaunchSite", "vents", "intervals", "turbine", "conduit"
  ],
            ...
            if (powerUps.powerUpStorage.length && m.alive && powerUp.length < 300) {
              ...
              if (!simulation.paused && !simulation.isChoosing) {
                if (!(simulation.cycle % delay)) {
                  const where = {
                    x: m.pos.x + 70 * (Math.random() - 0.5),
                    y: m.pos.y + 70 * (Math.random() - 0.5)
                  }
    ...
    if (document.getElementById("banned").value) { //remove levels from ban list in settings
      ...
      for (let i = 0; i < remove.length; i++) {
        ...
        if (index !== -1) {
          ...
          requestAnimationFrame(() => {
            simulation.inGameConsole(`banned level: <strong style="color: '#f00';">${remove[i]}</strong>`);
          });
        }
      }
  ...
  flipVertical() {
    const flipY = (who) => {
      for (let i = 0, len = who.length; i < len; i++) {
        Matter.Body.setPosition(who[i], {
          x: who[i].position.x,
          y: -who[i].position.y - player.position.y
        })
      }
    }
    flipY(map)
    ...
    if (angle !== Math.PI / 2) { //angle !== 3 * Math.PI / 2
      ...
      who = map[map.length] = Matter.Bodies.fromVertices(x + 50, y + 35, Vertices.fromPath("80 40 -80 40 -50 -40 50 -40"), {
        ...
        query() {
          ...
          const rayVector = Vector.add(this.position, Vector.rotate({
            x: 100,
            y: 0
          }, angle))
          query = (who) => {
            ...
            if (list.length > 0) {
              Matter.Body.setVelocity(list[0].bodyA, Vector.rotate({
                x: 1.21 * Math.sqrt(Math.abs(speed)),
                y: 0
              }, angle));
            }
          }
          ...
          if (list.length > 0) {
            Matter.Body.setVelocity(player, Vector.rotate({
              x: 1.21 * Math.sqrt(Math.abs(speed)),
              y: 0
            }, angle));
        ...
        if (tech.isHealAttract) { //send heals to next portal
          for (let i = 0; i < powerUp.length; i++) {
            if (powerUp[i].name === "heal" && Vector.magnitudeSquared(Vector.sub(powerUp[i].position, m.pos)) < 1000000) {
              Matter.Body.setPosition(powerUp[i], Vector.add(this.portalPair.portal.position, {
          ...
          let pushBlock = (who) => {
            if (!who.isMover) {
              ...
              Matter.Body.setVelocity(who, {
                x: this.VxGoal * (1 - stoppingFriction) + who.velocity.x * stoppingFriction,
                y: who.velocity.y
              });
  ...
  transport(x, y, width, height, VxGoal = -6, force = VxGoal > 0 ? 0.0005 : -0.0005) {
    ...
    const rect = body[body.length] = Bodies.rectangle(x, y, width, height, {
      ...
      stops: {
        left: x,
        right: x + 1000
      }, //this should probably be reset in the level code for the actual train stops
      trainStop() {
        if (this.isMoving) {
          this.move();
