...
const mainLevels = {
  ...
  final() {
    ...
    if (simulation.difficultyMode > 5)
      for (let i = 0; i < 8; i++) powerUps.spawn(4600 + 40 * i, -30, "ammo"); //extra ammo on why difficulty
    ...
    const infrastructure = (x, isInProgress = true) => {
      ...
      const stations = [ //update totalNumberOfStations as you add more stations 
        ...
        () => { //people movers
          ...
          const elevator0 = level.elevator(x - 1300, -1175, 175, 50, -1600, 0.011, {
            up: 0.01,
            down: 0.7
          })
          const elevator1 = level.elevator(x + 1125, -1175, 175, 50, -1600, 0.011, {
            up: 0.01,
            down: 0.7
          })
      ...
      if (isInProgress) {
        ...
        if (repositionTrain.position.x > playerOnTrain.position.x) { //decide if the train is moving left or right
          Matter.Body.setPosition(repositionTrain, {
            x: -1725 + x,
            y: repositionTrain.position.y
          });
          repositionTrain.changeDirection(false) //go left
          repositionTrain.stops = {
            right: -1725 + x,
            left: -7225 + x
          }
        ...
        if (button.isUp) {
          ...
          if (player.position.x > 0) {
            if (!doorOut.isClosed() || !doorIn.isClosed()) {
              ...
              if (simulation.difficultyMode > 5)
                for (let i = 0; i < 8; i++) powerUps.spawn(1200 + 550 * Math.random(), -1700, "ammo"); //extra ammo on why difficulty
  ...
  towers() {
    ...
    if (isFlippedHorizontal) level.fallModeBounds = {
      left: level.exit.x,
      right: level.enter.x
    } //used with level.fallMode = "position";
  ...
  labs() {
    ...
    lootOptions = [ //has some power up reward //field, ammo, research, gun
      (x = offset.x, y = offset.y) => {
        ...
        if (powerUp1) powerUp1.holdPosition = {
  ...
  interferometer() {
    ...
    let elevator = body[body.length] = Bodies.rectangle(800, 500, 300, 50, {
      ...
      move() {
            ...
            if (elevator.isUp) {
              ...
              if (elevator.position.y > -elevator.maxHeight) {
                ...
                Matter.Body.setPosition(elevator, {
                  x: elevator.holdX,
                  y: -elevator.maxHeight
                });
                Matter.Body.setVelocity(elevator, {
                  x: 0,
                  y: 0
                });
              }
        ...
        Matter.Body.setVelocity(elevator, {
          x: 0,
          y: elevator.velocity.y * this.drag
        }); //zero horizontal velocity and drag
        Matter.Body.setPosition(elevator, {
          x: elevator.holdX,
    ...
    let flipAndRemove = function() {
      ...
      function invertVertical(array) {
        for (let i = 0; i < array.length; ++i) {
          Matter.Body.setPosition(array[i], {
  ...
  substructure() {
    ...
    if (Math.random() < 0.33) {
      lasers[lasers.length - 1].oscillate = function() {
        this.look.x = 300 + Math.abs(600 * Math.sin(simulation.cycle * 0.017))
      }
    } else if (Math.random() < 0.5) {
      lasers[lasers.length - 1].oscillate = function() {
        this.look.x = 600 + 300 * Math.sin(simulation.cycle * 0.017)
      }
    } else {
      lasers[lasers.length - 1].oscillate = function() {
        this.look.x = 300 + (4 * simulation.cycle % 600)
      }
    }
    lasers.push(level.laser({
      x: 2375,
    ...
    level.custom = () => {
      ...
      if (buttonRight.isUp) {
        ...
        if (!buttonRight.isUp) {
          ...
          const list = Matter.Query.region(body, buttonLeft) //are any blocks colliding with this
          if (list.length > 0) Matter.Body.setVelocity(list[0], {
            x: baseMoverSpeed,
            y: -20
          });
        }
  ...
  flocculation() {
    ...
    //up mode triggered by player contact
    const elevator0 = level.elevator(700, 1865, 200, 490, 1400, 0.011, {
      up: 0.01,
      down: 0.7
    })
    const elevator1 = level.elevator(3995, 2335, 210, 150, 1700, 0.011, {
      up: 0.01,
      down: 0.7
    })
    ...
    level.customTopLayer = () => {
      ...
      hazard.level(
        ...
        button1.isUp, drainRate) //true = hold,  false = lower
