...
const removedLevels = {
  ...
  crimsonTowers() {
    ...
    function spawnTower(index, y = 0) {
      ...
      for (let i = 0; i < 5; i++) {
        if (Math.random() > 0.5) {
          ace.slasher2(index, y - (i * 500) - 500)
        } else {
          ace.slasher3(index, y - (i * 500) - 500)
        };
      }
    }
    // ace.slash(0, -5000);
  ...
  staircase() {
    ...
    var ddd = level.elevator(3399, -420, 200, 200, -950, 0.003, {
      up: 0.1,
      down: 0.2
  ...
  movers() {
    ...
    spawn.mapRect(1825, -2250, 3300, 300);
    spawn.mapRect(3250, -2875, 150, 625);
    ...
    spawn.mapRect(1850, -1950, 3275, 1275);
    spawn.mapRect(1850, -675, 3275, 1300);

    spawn.mapRect(2700, -2525, 25, 175);
    spawn.mapRect(3825, 925, 125, 575);
    spawn.mapRect(3600, 1100, 350, 400);
    spawn.mapRect(3375, 1350, 275, 150);
    spawn.mapRect(3550, 1300, 100, 50);
    spawn.mapRect(3800, 1000, 100, 150);
    spawn.mapRect(3725, 1075, 150, 125);
    spawn.mapRect(3725, 1025, 150, 125);
    spawn.mapRect(3550, 1225, 150, 125);
    spawn.mapRect(3500, 1275, 175, 125);
    ...
    var ddd = level.elevator(1326, -447, 200, 200, -2131, 0.003, {
      up: 0.1,
      down: 0.2
    });
    ...
    spawn.bodyRect(9200, 725, 50, 25);
    spawn.mapRect(12200, 675, 125, 50);
    spawn.mapRect(12925, 675, 100, 100);
    spawn.mapRect(13675, 650, 150, 150);
    spawn.mapRect(14200, 750, 25, 25);
    spawn.mapRect(14200, 675, 25, 75);
    spawn.mapRect(14550, 675, 125, 50);
    spawn.mapRect(15850, 675, 125, 100);
    spawn.mapRect(17175, 600, 25, 200);
    spawn.mapRect(17725, 700, 175, 50);
    spawn.mapRect(18775, 675, 175, 75);
    spawn.bodyRect(8975, 700, 25, 25);
    spawn.bodyRect(8850, 575, 50, 50);
    spawn.bodyRect(9050, 650, 50, 50);
    spawn.bodyRect(8625, 575, 100, 75);
    spawn.bodyRect(8475, 675, 75, 25);
  ...
  underpass() {
    ...
    const elevator = level.elevator(-11050, -650, 450, 75, -2975, 0.003, {
      up: 0.1,
      down: 0.1
    })
    ...
    const restoreBoss = function(x, y, radius = 30) { //ATTENTION LANDGREEN: RESTOREBOSS WILL NOT DROP ANY TECH, NOR WILL THERE BE ANY IN THE MAP. DO NOT ADD ANY TECH TO MY MAP
      ...
      me.do = function() {
        ...
        if (this.seePlayer.recall) {
          ...
          let positionR = {
            x: 500 * Math.cos(angle) + mob[0].position.x,
            y: 500 * Math.sin(angle) + mob[0].position.y
          };
        ...
        if (this.seePlayer.recall) {
          ...
          let positionR = {
            x: 5000 * Math.cos(angle) + mob[0].position.x,
            y: 5000 * Math.sin(angle) + mob[0].position.y
          };
      ...
      me.pushM = function() {
        ...
        if (tech.blockDmg) {
          Matter.Body.setVelocity(player, {
            x: 0.5 * player.velocity.x,
            y: 0.5 * player.velocity.y
          });
          //draw electricity
          const step = 40
          ctx.beginPath();
      ...
      me.pulse = function(charge, angle, where = this.position) {
        ...
        if (best.who) {
          ...
          }, explosionRadius, "rgba(0,0,0,0.7)")
          b.explosion({
            x: path[1].x + off * (Math.random() - 0.5),
            y: path[1].y + off * (Math.random() - 0.5)
          }, explosionRadius, "rgba(0,0,0,0.7)")
  ...
  cantilever() { // made by Eclipse#7932 on discord, (TheSpudguy)(@PurpleSunsetGames on github)
    ...
    spawn.bodyRect(80, -1300, 70, 1300, 1, {
      friction: .03,
      frictionAir: .001
    }); // cantilever
  ...
  dojo() { // By weird_pusheen
    ...
    const vanish = function(x, y, width, height) { // normal vanishes don't work well on my map for some reason, so I rewrote
      x += width / 2;
      y += height / 2;
      const getVertices = function(bX, bY, bW, bH) {
        return [{
          x: bX,
          y: bY,
          index: 0,
    ...
    mobs.spawn(500, -500, 10, 100, "yellow");
    /* TacticalBoss
               Modes:
                   Spawn:
    ...
    var spawnables = {};
    ["hopper", "stabber", "springer", "striker", "sneaker", "grower"].forEach((m) => {
      /* Used to be spawn.fullPickList, but some of those mobs don't do collision-only damage and would thus never be properly selected for */
      if (spawn[m]) {
        spawnables[m] = {
          fun: spawn[m],
          name: m,
    ...
    boss.do = () => {
      ...
      if (boss.mode == "hide") {
        /* Find the furthest point from M and get to it */
        var longest = 0;
  ...
  flappyGon() { //community map by digin
    ...
    setTimeout(() => {
      simulation.inGameConsole("<b>gravity</b> is a <b>choice</b>");
    }, 1000);
    setTimeout(() => {
      simulation.inGameConsole("everyone will fly");
    }, 2000);
    setTimeout(() => {
      simulation.inGameConsole("<b>jump from the post and find out</b>");
    }, 3000);
    level.setPosToSpawn(0, -50); //normal spawn
    level.exit.x = 8600;
    level.exit.y = -1100;
