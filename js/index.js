  ...
  generatePauseRight() {
    ...
    for (let i = 0, len = tech.tech.length; i < len; i++) {
      if (tech.tech[i].count > 0) {
        const style = (localSettings.isHideImages || tech.tech[i].isLore) ? `style="height:auto;"` : `style = "background-image: url('img/${tech.tech[i].name}.webp');"`
  ...
  if (simulation.testing) {
    ...
    switch (event.key.toLowerCase()) {
      ...
      case "j":
        ...
        setTimeout(() => {
          for (let i = 0, len = mob.length; i < len; ++i){
            if (!mob[i].isFinalBoss) mob[i].damage(Infinity, true)
          }
        }, 100);
        setTimeout(() => {
          for (let i = 0, len = mob.length; i < len; ++i){
...
document.getElementById("updates").addEventListener("toggle", function () {
  ...
  let text = `<pre><strong>n-mod</strong>: <a href="todo.txt">todo list</a><hr>`
  
  let txt = new XMLHttpRequest();
  txt.onloadend = function(oEvent) {
    try {
      if (txt.status === 200) {
        text += txt.responseText + "<hr>"
      } else {
        console.error("ERROR FETCHING TEXT FILE: ",(txt.statusText || "Exit status "+txt.status));
        text += `<strong style='color:red'>FAILED TO LOAD TODO LIST:<br>${(txt.statusText || "Exit status "+txt.status)}</strong><hr>`
      }
    } catch (error) {
      console.error("ERROR FETCHING TEXT FILE: ",error);
      text += `<strong style='color:red'>FAILED TO LOAD TODO LIST:<br>${error}</strong><hr>`
    }
    document.getElementById("updates-div").innerHTML = text.replace(/\n/g, "<br />") + "<hr>"
  };
  txt.open("GET", 'todo.txt', true);
  txt.send();
