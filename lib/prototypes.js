...
class ErrorType { //NOT to be confused with TypeError
  constructor(name) {
...
const ArgumentError = new ErrorType('Argument')
const HtmlError = new ErrorType('Html')
const HttpError = new ErrorType('Http')
Object.freeze(ArgumentError)
Object.freeze(HtmlError)
Object.freeze(HttpError)

Math.PHI = Math.sqrt(1.25) + 0.5 //defines the golden ratio

const hsvo = (hue = 0, saturation = 0, value = 0, opacity = 1) => { //generates an RGB string from hue, saturation, and value. Preserves opacity
  if (isFinite(hue)) {
    hue = ((hue % 360) + 360) % 360
  } else {
    hue = 0
  ...
  value = clamp(value)
  opacity = (opacity && isFinite(opacity) ? clamp(opacity) : 1)
...
const setTimeOuts = (funcs, delays) => { //replaces setTimeout nesting
    ...
    let iter = (f, d, m, n) => {
      if (n < m) {
        if (d[n] < 10) {
          ...
          setTimeout(() => { f[n](); iter(f, d, m, n+1); }, d[n])
        }
      }
    }
    iter(funcs, delays, funcs.length, 0)
  } else {
    throw new TypeError(`Cannot get properties of undefined (reading arrays of different lengths)`)
  }
}
Object.freeze(setTimeOuts)

Object.prototype.tryNoCatch = function () {
  try { return this(); } catch(e) { /*return e*/ }
}

Object.prototype.unassignAll = function () {
  ((t)=>{Object.keys(t).forEach((i)=>{delete this[i]},t)})(this)
}

Array.fromRange = (start = 0, end = 10, step = 1) => {
  return ((J, K, L, M) => {
    return (J == K ? M : ((j, k, l, m) => {
      for(let i=j,n=Math.abs(l)*Math.sign(k-j);Math.abs(i-j)<=Math.abs(k-j);i+=n)m.push(i);
      return m
    })(J, K, L, M))
  })(start, end, step, [])
}

Array.toEntries = (keys = [], values = []) => keys.map((i, j) => [i, values[j]])

Array.prototype.depth = function () {
  let iter = (i) => (Array.isArray(i) ? 1 + Math.max(0, ...i.map(iter)) : 0)
  return iter(this.valueOf())
}

document.getElementsByIds = (...ids) => {
  return ((k) => {
    k.forEach((i,j) => {
      k[j] = document.getElementById(i)
    })
    return k
  })(ids)
}
...
Object.prototype.getValues = function (...keys) {
  return keys.valueOf().map(i => this[i])
}

Boolean.OR = (...v) => v.some(i => i)
Boolean.AND = (...v) => v.every(i => i)
Boolean.XOR = (...v) => (v.some(i => !i) && v.some(i => i))
Boolean.NOR = (...v) => v.every(i => !i)
Boolean.NAND = (...v) => v.some(i => !i)
Boolean.XNOR = (...v) => (v.every(i => !i) || v.every(i => i))

String.areAnagrams = (...v) => {
  let a = v.valueOf().map(i => i.split('').sort().join('')), b = v.valueOf().map(i => i.length)
  return (a.every(i => i.toLowerCase() == a[0].toLowerCase()) && b.every(i => i == b[0]))
}

//a lot of these aren't even prototypes lol